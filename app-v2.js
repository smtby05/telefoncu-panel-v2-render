(function () {
  ...
  function shell(content) {
    ...
    const visibleNav = simple ? nav.filter(x => ["dashboard","services","customers","stock","sales","cash"].includes(x[0])) : nav;
    ...
    return `<div class="layout">
      <aside class="sidebar" id="sidebar"><div class="brand"><span class="brand-mark">T</span><div>${esc(db.settings.businessName || "İşletme Paneli")}<small>Servis ve satış yönetimi</small></div></div>
        <nav class="nav">${visibleNav.map(([r,n]) => `<button data-go="${r}" class="${route.startsWith(r) ? "active" : ""}">${r==="users"&&!isAdmin()?"Kendi İşlerim":n}</button>`).join("")}</nav>
      </aside>
...
function denied(){return page("Yetki Gerekli","Bu alan hesabınıza açılmamış.","",`<div class="empty-state"><strong>Erişim izniniz yok</strong>İşletme Sahibi hesabından bu kullanıcıya gerekli izin verilebilir.</div>`);}
  ...
  function auditMatchesUser(entityId,userId,actions=[]){
    return db.auditLogs.some(log=>log.userId===userId&&log.entityId===entityId&&(!actions.length||actions.some(action=>String(log.action||"").includes(action))));
  }
  function recordBelongsToUser(record,userId,actions=[]){
    if(!record)return false;
    return [record.staffId,record.createdById,record.sellerId,record.soldById,record.userId,record.uploadedById].includes(userId)||auditMatchesUser(record.id,userId,actions);
  }
  function userWorkData(userId){
    const services=db.services.filter(x=>recordBelongsToUser(x,userId,["Servis oluşturma"]));
    const sales=db.sales.filter(x=>recordBelongsToUser(x,userId,["Satış"]));
    const purchases=db.buySell.filter(x=>x.createdById===userId||auditMatchesUser(x.id,userId,["Al-sat cihaz alımı"]));
    const sold=db.buySell.filter(x=>x.soldById===userId||(!x.soldById&&auditMatchesUser(x.id,userId,["Al-sat satışı"])));
    const activities=db.auditLogs.filter(x=>x.userId===userId);
    const serviceIncome=services.reduce((sum,x)=>sum+Number(x.paid||0),0);
    const accessorySales=sales.filter(x=>!x.buySellId);
    const accessoryRevenue=accessorySales.reduce((sum,x)=>sum+Number(x.total||0),0);
    const accessoryCost=accessorySales.reduce((sum,sale)=>sum+(sale.items||[]).reduce((itemSum,item)=>{const stock=db.stock.find(x=>x.id===item.stockId);return itemSum+Number(item.buyPrice??stock?.buyPrice??0)*Number(item.quantity||0);},0),0);
    const accessoryProfit=accessoryRevenue-accessoryCost;
    const buySellProfit=sold.reduce((sum,x)=>sum+Number(x.actualProfit||0),0);
    return {services,sales,purchases,sold,activities,serviceIncome,accessoryRevenue,accessoryProfit,buySellProfit,totalEarning:serviceIncome+accessoryProfit+buySellProfit};
  }
  function userWorkBody(target){
    const d=userWorkData(target.id),recentServices=d.services.slice(0,20),recentSales=d.sales.slice(0,20),recentActivity=d.activities.slice(0,30);
    return `<section class="panel"><div class="page-head"><div><h2>${esc(target.fullName)}</h2><p>${esc(roleLabel(target.role))} · @${esc(target.username)} · ${target.active?"Aktif":"Pasif"}</p></div>${target.id===user.id?button("Hesabımı Yönet","goAccount"):""}</div></section>
      <div class="stats grid">${stat("Açtığı Servis",d.services.length,"blue")}${stat("Teslim Ettiği Servis",d.services.filter(x=>x.status==="TESLIM_EDILDI").length,"green")}${stat("Yaptığı Satış",d.sales.length,"gray")}${stat("Aldığı Cihaz",d.purchases.length,"orange")}${stat("Sattığı Al-Sat Cihazı",d.sold.length,"green")}${stat("Hesaplanan Kazanç",money(d.totalEarning),d.totalEarning<0?"red":"green")}</div>
      <section class="grid split"><div class="panel"><h2>Kazanç Dağılımı</h2><p>Servis tahsilatı: <strong>${money(d.serviceIncome)}</strong></p><p>Aksesuar tahmini brüt kârı: <strong>${money(d.accessoryProfit)}</strong></p><p>Al-Sat gerçek kâr/zararı: <strong>${money(d.buySellProfit)}</strong></p><p class="hint">Servislerde parça ve işçilik maliyeti ayrı girilmediği için servis rakamı tahsilat olarak gösterilir.</p></div><div class="panel"><h2>Hesap Bilgisi</h2><p>Son giriş: <strong>${fmt(target.lastLoginAt)}</strong></p><p>Durum: ${badge(target.active?"AKTİF":"PASİF")}</p><p>Rol: ${badge(roleLabel(target.role))}</p></div></section>
      <section class="panel"><h2>Bu Hesapla Açılan Cihazlar / Servisler</h2>${recentServices.length?table(["Tarih","Servis No","Cihaz","Durum","Tahsilat"],recentServices.map(x=>[fmt(x.createdAt),esc(x.no),`${esc(x.brand)} ${esc(x.model)}`,badge(x.status),money(x.paid||0)])):`<div class="empty-state"><strong>Servis kaydı yok</strong>Bu kullanıcıyla açılmış cihaz kaydı bulunmuyor.</div>`}</section>
      <section class="panel"><h2>Satış ve Al-Sat İşleri</h2>${recentSales.length?table(["Tarih","Satış No","Tutar","Ödeme"],recentSales.map(x=>[fmt(x.createdAt),esc(x.no||"-"),money(x.total),esc(x.payment||"-")])):`<div class="empty-state"><strong>Satış kaydı yok</strong>Bu kullanıcıya bağlı satış bulunmuyor.</div>`}</section>
      <section class="panel"><h2>Son İşlemler</h2>${recentActivity.length?table(["Tarih","İşlem","Kayıt Türü"],recentActivity.map(x=>[fmt(x.createdAt),esc(x.action),esc(x.entityType)])):`<div class="empty-state"><strong>İşlem geçmişi yok</strong>Bu kullanıcı henüz işlem yapmamış.</div>`}</section>`;
  }
  function userManagementButtons(target){
    const activity=button("Faaliyet Detayı",`userWork:${target.id}`,"secondary");
    if(target.id===user.id)return `${activity}${button("Hesabımı Yönet","goAccount","secondary")}`;
    if(target.role==="OWNER")return `${activity}<span class="hint"> İşletme Sahibi hesabı korumalı</span>`;
    const canDelete=user.role==="OWNER"||(user.role==="ADMIN"&&target.role!=="ADMIN");
    return `${activity}${button("Yetkisini Değiştir",`roleUser:${target.id}`,"secondary")}${target.role==="ADMIN"?"":button("Yetkileri Düzenle",`permissions:${target.id}`,"secondary")}${button(target.active?"Kullanıcıyı Pasifleştir":"Kullanıcıyı Aktifleştir",`toggleUser:${target.id}`,"secondary")}${button("Şifre Sıfırla",`resetUser:${target.id}`,"secondary")}${canDelete?button("Kullanıcıyı Sil",`deleteUser:${target.id}`,"danger"):""}`;
  }
  function users() {
    if(!isAdmin())return page("Kendi İşlerim","Bu hesapla açılan cihazları, satışları ve işlem geçmişini görürsünüz.","",userWorkBody(user));
    ...
    return page("Kullanıcılar","Yalnızca yönetici hesapları rol, yetki, şifre, askıya alma ve silme işlemlerini görür.",button("Yeni Kullanıcı","userModal"),
      requestsPanel+
      table(["Ad Soyad","Kullanıcı","Rol","Servis","Satış","Kazanç","Son Giriş","Durum","İşlemler"],db.users.map(x=>{const d=userWorkData(x.id);return[esc(x.fullName),esc(x.username),badge(roleLabel(x.role)),d.services.length,d.sales.length,money(d.totalEarning),fmt(x.lastLoginAt),badge(x.active?"AKTİF":"PASİF"),userManagementButtons(x)];})));
  }
  function userWorkDetail(i){if(!isAdmin()&&i!==user.id)return toast("Yalnızca kendi işlerinizi görebilirsiniz.","error");const target=db.users.find(x=>x.id===i)||(i===user.id?user:null);if(!target)return toast("Kullanıcı bulunamadı.","error");modal("Kullanıcı Faaliyet Detayı",userWorkBody(target));}
  function account() {
  ...
  async function handle(action, el) {
    try {
      ...
      if(action==="goAccount"){$$(".modal-backdrop").forEach(x=>x.remove());return go("account");}
...
if(action==="ownerOverride")return toast("İşletme Sahibi onayı bu oturum için hazır. Zorunlu işlemde yetkili kullanıcı devam edebilir.");
      ...
      if(action.startsWith("userWork:"))return userWorkDetail(action.split(":")[1]);
  ...
  function saveService(print){const f=$("#serviceForm"),d=formData(f);if(!d.customerName||!d.phone||!d.brand||(!d.model&&!d.manualModel)||!d.issue)return toast("Müşteri, cihaz ve arıza bilgilerini doldurun.","error");let c=db.customers.find(x=>x.phone===d.phone);if(!c){c={id:id("CUS"),name:d.customerName,phone:d.phone,address:d.address,createdById:user.id,createdAt:iso()};db.customers.unshift(c);}const model=d.model==="MANUAL"?d.manualModel:d.model,no=`SRV-${new Date().getFullYear()}-${String(db.counters.service++).padStart(6,"0")}`,s={id:id("SRV"),no,customerId:c.id,brand:d.brand,model,storage:d.storage,ram:d.ram,color:d.color,imeiOrSerial:d.imeiOrSerial,issue:d.issue,price:+d.price,paid:+d.prepayment,warrantyDays:+d.warrantyDays,status:"TAMIRDE",notes:[d.note].filter(Boolean),staffId:user.id,branchId:d.branchId,trackingToken:crypto.randomUUID(),createdAt:iso(),updatedAt:iso()};db.services.unshift(s);if(+d.prepayment)db.cash.unshift({id:id("CSH"),type:"IN",amount:+d.prepayment,title:`${no} ön ödeme`,method:"Nakit",ref:no,createdById:user.id,at:iso()});audit("Servis oluşturma","SERVİS",s.id,null,s);save();toast(`${no} servis kaydı oluşturuldu.`);if(print)printData("Servis Teslim Formu",s);go("services");}
  ...
  function customerModal(){const m=modal("Yeni Müşteri",`<form id="customerForm"><div class="form-grid"><label>Ad soyad<input name="name" required></label><label>Telefon<input name="phone" required></label><label class="wide">Adres<input name="address"></label><label class="full">Not<textarea name="note"></textarea></label></div></form>`,button("Kaydet","saveCustomer"));$("[data-action=saveCustomer]",m).onclick=()=>{const d=formData($("#customerForm"));if(db.customers.some(x=>x.phone===d.phone))return toast("Bu telefonla kayıtlı müşteri var.","error");db.customers.unshift({id:id("CUS"),...d,createdById:user.id,createdAt:iso()});audit("Müşteri oluşturma","MÜŞTERİ");save();m.remove();render();toast("Müşteri kaydedildi.");};}
  ...
  function completeSale(){const d=formData($("#saleForm")),x=db.stock.find(s=>s.id===d.productId||s.barcode===d.barcode),qty=+d.quantity;if(!x||x.quantity<qty)return toast("Ürün bulunamadı veya stok yetersiz.","error");const total=Math.max(0,x.salePrice*qty-Number(d.discount||0)),sale={id:id("SAL"),no:`SAT-${String(db.counters.sale++).padStart(6,"0")}`,customerId:d.customerId,items:[{stockId:x.id,name:x.name,quantity:qty,price:x.salePrice,buyPrice:Number(x.buyPrice||0)}],total,payment:d.payment,staffId:user.id,createdById:user.id,createdAt:iso()};db.sales.unshift(sale);x.quantity-=qty;db.stockMovements.unshift({id:id("SM"),stockId:x.id,title:x.name,type:"OUT",quantity:qty,createdById:user.id,createdAt:iso()});if(d.payment==="Veresiye")db.receivables.unshift({id:id("RCV"),customerId:d.customerId,title:`${sale.no} veresiye`,total,paid:0,dueDate:day(),status:"BEKLIYOR",createdById:user.id,createdAt:iso()});else db.cash.unshift({id:id("CSH"),type:"IN",amount:total,title:`${sale.no} satış`,method:d.payment,ref:sale.no,createdById:user.id,at:iso()});audit("Satış","SATIŞ",sale.id);save();render();toast("Satış tamamlandı.");}
...
function userModal(){if(!isAdmin())return toast("Bu işlem yalnızca yönetici hesabına açıktır.","error");const allowedRoles=user.role==="OWNER"?roles:roles.filter(x=>x!=="OWNER");const m=modal("Kullanıcı Oluştur",`<form id="userForm"><div class="form-grid"><label>Ad soyad<input name="fullName" required></label><label>Kullanıcı adı<input name="username" required></label><label>E-posta<input name="email" type="email" required></label><label>Telefon<input name="phone"></label><label>Geçici şifre<input name="password" type="password" required></label><label>Görevi<select name="role">${roleOptions(allowedRoles)}</select></label></div></form>`,button("Kullanıcı Oluştur","saveUser"));$("[data-action=saveUser]",m).onclick=async()=>{const d=formData($("#userForm"));if(!validPassword(d.password))return toast("Şifre kurallara uymuyor.","error");if(db.users.some(x=>x.username===d.username||x.email===d.email))return toast("Kullanıcı adı veya e-posta kullanımda.","error");let created;if(isServer){try{created=(await api("/users",{method:"POST",body:JSON.stringify(d)})).user;}catch(e){return toast(e.message,"error");}}else created={id:id("USR"),fullName:d.fullName,username:d.username,email:d.email,phone:d.phone,role:d.role,active:true,permissions:d.role==="OWNER"?["*"]:[],passwordHash:await hashPassword(d.password),createdAt:iso()};db.users.unshift(created);audit("Kullanıcı oluşturma","KULLANICI",created.id);save();m.remove();render();toast("Kullanıcı oluşturuldu.");};}
function permissionsModal(i){if(!isAdmin())return toast("Bu işlem yalnızca yönetici hesabına açıktır.","error");const x=db.users.find(x=>x.id===i);if(!x)return toast("Kullanıcı bulunamadı.","error");if(["OWNER","ADMIN"].includes(x.role))return toast("İşletme Sahibi ve Yönetici rolleri tam yetkilidir; tek tek izinleri kapatılamaz.","error");const m=modal("Kullanıcı Yetkileri",`<p><strong>${esc(x.fullName)}</strong></p><div class="permission-grid">${permissions.map(([key,desc])=>`<label><input type="checkbox" value="${esc(key)}" ${(x.permissions||[]).includes(key)?"checked":""}>${esc(desc)} <small>${esc(key)}</small></label>`).join("")}</div>`,button("Yetkileri Kaydet","savePermissions"));$("[data-action=savePermissions]",m).onclick=async()=>{x.permissions=$$(".permission-grid input:checked",m).map(c=>c.value);if(isServer)try{await api(`/users/${x.id}`,{method:"PATCH",body:JSON.stringify({permissions:x.permissions})});}catch(e){return toast(e.message,"error");}audit("Kullanıcı yetkileri değiştirildi","KULLANICI",x.id);save();m.remove();render();toast("Yetkiler güncellendi.");};}
async function toggleUser(i){if(!isAdmin())return toast("Bu işlem yalnızca yönetici hesabına açıktır.","error");const x=db.users.find(x=>x.id===i);if(!x)return toast("Kullanıcı bulunamadı.","error");if(x.id===user.id)return toast("Kendi hesabınızı pasifleştiremezsiniz.","error");if(x.role==="OWNER")return toast("İşletme Sahibi hesabı pasifleştirilemez.","error");const active=!x.active;if(!(await confirmAction(`${x.fullName} hesabı ${active?"aktifleştirilsin":"askıya alınsın"} mı?`)))return;if(isServer)try{await api(`/users/${i}`,{method:"PATCH",body:JSON.stringify({active})});}catch(e){return toast(e.message,"error");}x.active=active;audit(active?"Kullanıcı aktifleştirildi":"Kullanıcı askıya alındı","KULLANICI",i);save();render();toast(active?"Kullanıcı aktifleştirildi.":"Kullanıcı askıya alındı.");}
function roleUser(i){if(!isAdmin())return toast("Bu işlem yalnızca yönetici hesabına açıktır.","error");const x=db.users.find(x=>x.id===i);if(!x)return toast("Kullanıcı bulunamadı.","error");if(x.id===user.id)return toast("Kendi görevinizi buradan değiştiremezsiniz.","error");if(x.role==="OWNER")return toast("İşletme Sahibi yetkisi düşürülemez.","error");const allowed=user.role==="OWNER"?roles:roles.filter(r=>r!=="OWNER"),m=modal("Yetkisini Değiştir",`<label>Görevi<select id="newRole">${allowed.map(r=>`<option value="${r}" ${r===x.role?"selected":""}>${roleLabel(r)}</option>`).join("")}</select></label>`,button("Görevi Güncelle","saveRole"));$("[data-action=saveRole]",m).onclick=async()=>{const role=$("#newRole").value;if(x.role==="OWNER"&&role!=="OWNER")return toast("İşletme Sahibi görevi değiştirilemez.","error");if(isServer)try{await api(`/users/${i}`,{method:"PATCH",body:JSON.stringify({role})});}catch(e){return toast(e.message,"error");}x.role=role;if(role==="OWNER")x.permissions=["*"];audit("Kullanıcı görevi değiştirildi","KULLANICI",i);save();m.remove();render();toast("Kullanıcı görevi güncellendi.");};}
async function deleteUser(i){if(!isAdmin())return toast("Bu işlem yalnızca yönetici hesabına açıktır.","error");const x=db.users.find(x=>x.id===i);if(!x)return toast("Kullanıcı bulunamadı.","error");if(user.role==="ADMIN"&&x.role==="ADMIN")return toast("Yönetici hesabını yalnızca İşletme Sahibi silebilir.","error");if(x.role==="OWNER")return toast("İşletme Sahibi hesabı silinemez.","error");if(!(await confirmAction(`${x.fullName} kullanıcısı silinsin mi?`)))return;if(isServer)try{await api(`/users/${i}`,{method:"DELETE"});}catch(e){return toast(e.message,"error");}db.users=db.users.filter(u=>u.id!==i);audit("Kullanıcı silindi","KULLANICI",i);save();render();toast("Kullanıcı silindi.");}
function resetUser(i){if(!isAdmin())return toast("Bu işlem yalnızca yönetici hesabına açıktır.","error");const x=db.users.find(x=>x.id===i);if(!x)return toast("Kullanıcı bulunamadı.","error");if(x.id===user.id)return toast("Kendi şifrenizi Hesabım ekranından değiştirin.","error");if(x.role==="OWNER")return toast("İşletme Sahibi şifresini yalnızca kendisi değiştirebilir.","error");const m=modal("Şifre Sıfırla",`<form id="resetForm"><label>Yeni geçici şifre<input name="password" type="password" required></label></form>`,button("Şifreyi Sıfırla","saveReset"));$("[data-action=saveReset]",m).onclick=async()=>{const p=formData($("#resetForm")).password;if(!validPassword(p))return toast("Şifre kurallara uymuyor.","error");if(isServer)try{await api(`/users/${i}/reset-password`,{method:"POST",body:JSON.stringify({password:p})});}catch(e){return toast(e.message,"error");}else x.passwordHash=await hashPassword(p);audit("Yönetici şifre sıfırlama","KULLANICI",i);save();m.remove();toast("Kullanıcı şifresi sıfırlandı.");};}
